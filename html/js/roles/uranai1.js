$(function() {
    ...
    var nWerewolf = localStorage.getItem(300) - '0';
    var nUranaishi = localStorage.getItem(201) - '0';
    var names = [];
    var roles = [];
    ...
    //占い結果
    var nNoWolf = n - nWerewolf - nUranaishi;
    if(uranai == -1)
    {
        var rand = Math.floor(Math.random()*1027)%nNoWolf;
        var noWolfs = [];
        for(var i = 0; i < n; i++)
        {
            if(roles[i] != 300 && roles[i] != 201)
                noWolfs.push(i+1);
        }
        uranai = noWolfs[rand];
        localStorage.setItem("uranai", uranai)
    }
    $("#uranaiRandom").append("<div class=\"roles\"><p id=\"p" + i + "\">" + names[uranai-1] + "</p><p class=\"deathp\">人狼ではない</p></div>");
